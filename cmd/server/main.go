...
func main() {
	...
	flag.StringVar(&usersPath, "users-path", "", "path to users")
	flag.Parse()
	...
	users := map[string]server.UserInfo{}
	if usersPath != "" {
		data, err := os.ReadFile(usersPath)
		if err != nil {
			log.Fatalf("read users: %s", err)
		}
